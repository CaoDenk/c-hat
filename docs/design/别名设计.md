...
`using` 的右侧支持 `A | B | C` 语法，表示**编译期类型集合约束**，是 Concept 约束的简写形式。

```c^
using Numeric = int | long | float | double;
using Integral = int | long | byte;
using FloatLike = float | double;
```

用于泛型 `where` 子句，限制类型参数必须是集合中的某一个：

```c^
func sum<T>(T[] values) -> T where Numeric<T> {
    // T 必须是 int, long, float, double 之一
}

func clamp<T>(T val, T lo, T hi) -> T where Integral<T> { ... }
```

**严格限制：**

- `A | B | C` **只能**出现在 `using` 右侧，或直接用于 `where` 子句
- **不能**作为变量类型：`Numeric x = 1;` → 编译错误
- **不能**用于 `match` 解构（不是运行时和类型）
- **不能**作为函数参数类型：`func foo(Numeric x)` → 编译错误
- 本质是 Concept 约束的语法糖，**无运行时开销，无内存布局影响**
...
// 使用 | 语法糖（简洁）
using Numeric = int | long | float | double;
func sum<T>(T[] v) -> T where Numeric<T> { ... }

// 等价的 Concept 写法（显式）
concept Numeric<T> where
    typeof(T) == typeof(int)   ||
