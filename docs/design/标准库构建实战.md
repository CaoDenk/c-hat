...
| 特性分类     | 内置特性               | 作用           | 理由                                             |
| :----------- | :--------------------- | :------------- | :----------------------------------------------- |
| **物理类型** | `literalview`         | 只读代码段视图 | 字符串字面量的物理载体，构建 `String` 的原材料。 |
...
### 2.1 需求
我们需要一个类型，能同时指向 `int[5]` (栈数组) 和 `literalview` (静态区)，并提供下标访问。
...
public struct Slice<T> {
    ...
    // 协议构造：支持从 literalview 隐式转换
    public implicit Slice(literalview lv) 
...
1. 编译器生成 `literalview` (指向 .rodata)。
2. 编译器查找 `Slice` 是否有接受 `literalview` 的 `implicit constructor`。
3. 发现匹配，生成调用代码。
...
public struct String {
    ...
    // 状态位：最高位 1 表示 Static (literalview)，0 表示 Heap
    private usize capacity_and_flags; 

    // 1. 从字面量构造 (零拷贝)
    // 需要 unsafe，因为我们将只读的 literalview 强转为 Slice<byte> (内部是 byte^)
    // 这是安全的，因为 capacity_and_flags 标记了 FLAG_STATIC，析构和写入时会检查
    public implicit unsafe String(literalview lv) {
        // 显式转换指针类型
...
**结论**：`String` 完全是用户态代码。只要语言提供 `literalview` (来源) 和 `destructor` (清理)，就能构建出高效的字符串类。
...
1.  **数据布局**：`struct`, `array`, `ptr`, `literalview`。
