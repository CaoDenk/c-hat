...
**数组和切片都有 `ptr` 属性，指向数据首地址的指针**：

```c^
// ========== 数组的 ptr 属性 ==========
int[3] arr = [1, 2, 3];
println(arr.ptr);    // 指向 arr[0] 的指针（编译期常量地址，零开销）

// ========== 切片的 ptr 属性 ==========
int[] slice = arr;
println(slice.ptr);  // 指向 arr[0] 的指针，与 arr.ptr 值相同

// ========== 只读数组/切片的 ptr 属性 ==========
int[3]! const_arr = [1, 2, 3];
println(const_arr.ptr);  // 指向 const_arr[0] 的 const 指针（byte!^）

int![] const_slice = const_arr;
println(const_slice.ptr); // 同样是 const 指针
```

**设计说明：**
- `ptr` 是只读属性（get-only），不能修改
- **数组也有 `ptr` 属性**：直接指向数组首地址，这是编译期已知的值，零额外开销
- 切片的 `ptr` 属性指向切片引用的数据首地址
- 只读数组/切片的 `ptr` 返回 `const` 指针（`T!^`）

> **为什么数组也有 `ptr`？**
> 1. **C 互操作**：传数组给 C 函数时 `arr.ptr` 比 `^arr[0]` 语义更清晰
> 2. **一致性**：`len` 数组切片都有，`ptr` 也应对称
> 3. **零开销**：数组首地址是编译期已知的，不需要额外存储
> 4. **等价性**：`arr.ptr` 与 `^arr[0]` 语义完全等价，但前者更表达意图
...
当数组隐式转换为切片时，两者共享相同的 `ptr`（指向同一块内存）：
...
// 切片的 ptr 指向 arr 的内存（只读）
println(slice.ptr); // 指向 arr[0] 的指针
println(arr.ptr);   // 与 slice.ptr 值相同

// arr.ptr == slice.ptr 为 true（指向同一块内存）

// 需要显式取某个元素地址时也可以：
int^ p = ^arr[0];   // 与 arr.ptr 等价，但语义更具体
```

**设计说明：**
- 数组和切片都有 `ptr` 属性，都是只读（get-only）
- 数组转切片后，`arr.ptr == slice.ptr`，无拷贝，零开销
- 通过 `^arr[i]` 可以取任意元素的地址（`arr.ptr` 等价于 `^arr[0]`）
...
**A:**
- **C 互操作方便**：传数组给 C 函数时 `arr.ptr` 比 `^arr[0]` 语义更清晰直接
- **一致性**：`len` 数组切片都有，`ptr` 也对称支持，减少"这个有没有"的心智负担
- **零开销**：数组首地址是编译期已知的常量，不需要额外存储字段
- `arr.ptr` 与 `^arr[0]` 完全等价，前者更表达意图
