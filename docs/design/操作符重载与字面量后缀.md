...
```cpp
// 标准库实现示例 (伪代码)
public operator"" fp(long double x) -> fp16 {
    return fp16(x);
}
```
...
```cpp
// 定义后缀 km (千米) -> 米
public operator"" km(long double x) -> double {
    return (double)x * 1000.0;
}

// 定义后缀 s (秒) -> 秒
public operator"" s(unsigned long long x) -> double {
    return (double)x;
}
...
// 1. 定义字符串前缀 json"..."
// 接收参数：原始 UTF-8 字节数组
public static operator "json"(byte[] raw) -> JsonObject {
    return JsonParser.parse(string_view(raw));
}

// 2. 定义字符前缀 u8'...'
public static operator "u8"(char code_point) -> byte {
...
public static operator "$xml"(usize len, usize args) -> XmlHandler {
...
class Vector2 {
    ...
    // 1. 二元操作符：+
    // 语义：只读自身 (self!)，返回新对象
    public operator+(self!, Vector2 rhs) -> Vector2 {
        return Vector2(self.x + rhs.x, self.y + rhs.y);
    }

    // 2. 复合赋值：+=
    // 语义：修改自身 (self)
    public operator+=(self, Vector2 rhs) {
        self.x += rhs.x;
        self.y += rhs.y;
    }
    ...
    // 3. 判等：==
    // 语义：只读自身 (self!)
    public operator==(self!, Vector2 rhs) -> bool {
        return self.x == rhs.x && self.y == rhs.y;
    }
}
```

> **设计说明**：操作符重载不需要 `func` 关键字。`operator+` 本身已经清楚地表明这是一个操作符重载，再加 `func` 是冗余的。这与 C#、C++ 等语言的设计一致。
...
class Array<T> {
    // 1. 读操作：arr[i]
    // 语义：self! (只读)
    public operator[](self!, int index) -> T {
        return _data[index];
    }
...
class Adder {
    // 语义：self! (无状态修改)
    public operator()(self!, int a, int b) -> int {
        return a + b;
    }
}
```
...
class Wrapper<T> {
    ...
    public operator->(self!) -> UniquePtr<T> {
        return self._uptr;
    }
}
...
*   **Prefix (前缀)**：`op x`
    ...
    *   **逻辑**：`void` 在左，`x` 在右。
    *   **成员函数**：`operator op(void, self)`

*   **Suffix (后缀)**：`x op`
    *   **映射**：`operator op(x, void)`
    *   **逻辑**：`x` 在左，`void` 在右。
    *   **成员函数**：`operator op(self, void)`
...
class Number {
    ...
    public operator++(void, self) -> Number {
        self.val += 1;
        return self;
    }
    ...
    public operator++(self, void) -> Number {
        var temp = self;
        self.val += 1;
        return temp;
    }
...
1.  **成员函数 (Member Function) —— 推荐默认方式**
    ...
    *   **示例**：
        ...
        class Vector {
            // 成员函数：self! + rhs
            public operator+(self!, Vector rhs) -> Vector { ... }
        }
        ```
...
2.  **静态函数 (Static Function) / 扩展方法**
    ...
    *   **示例**：
        ```cpp
        // 静态扩展：允许 2.0 * vec
        public static operator*(double lhs, Vector rhs) -> Vector {
...
class Math {
    ...
    public static operator**(double base, double exp) -> double {
...
*   **操作符重载**：语法简洁直观（无需 `func` 关键字），支持成员与全局两种形式，严格限制可重载集合以防滥用。
