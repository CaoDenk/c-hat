...
*   **方式 1：函数后缀 `!`**（推荐，简洁）
*   **方式 2：`self!` 参数**（显式，明确表达意图）

*   **语义**：在不可变方法体内，无法修改成员变量。
...
* **语法**：在类内部定义的函数
* **self 设计**：
    * **普通方法**：`self` 可选，不关心可变性时可省略
    * **标记只读**：
        * **函数后缀 `!`**：`func foo()! { }`（推荐，简洁）
        * **`self!` 参数**：`func foo(self!) { }`（显式风格）
    * **运算符重载**：强制使用 `self`（或 `self!`）
* **self 语义**：指代当前对象实例（引用或值，而非指针）
* **调用方式**：通过对象实例调用
...
public:
    ...
    // 不可变方法：self! 参数 (显式风格)
    public int sum(self!, int a, int b) {
        return a + b;
    }
