...
| **L0** | **语言公理层** (Axioms) | `int`, `bool`, `ptr`, `array`, `literalview` | **无依赖** | 编译器内置原语，物理事实 |
...
*   **作用**：它统一了对 `T[N]` (栈数组) 和 `literalview` (静态数据) 的访问方式。
...
struct String {
    ...
    // 依赖 L0 的 literalview 进行初始化
    public String(literalview lit) {
...
1.  **L0 (公理)** 提供了物理材料：内存块 (`array`)、地址 (`ptr`)、常量数据 (`literalview`)。
