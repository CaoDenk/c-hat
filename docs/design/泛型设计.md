...
func sort<T>(List<T> list) where Comparable<T> { ... }
...
当约束仅需限定类型参数必须是某几个类型之一时，可以用 `using` 的 `|` 语法定义类型集合，作为 Concept 的简写：
...
func abs<T>(T x) -> T where Integral<T> { ... }
```

**限制**：`|` 表达式是纯编译期约束，不能作为变量类型或函数参数类型，只能出现在 `using` 右侧或 `where` 子句中。等价的显式 Concept 写法：
...
#### 2.3.1 简单表达式需求 (Simple Requirement)
验证表达式是否合法（能编译通过）。
