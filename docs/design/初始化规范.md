...
在 `return` 语句中，同样需要遵循类型明确性原则：

1. **基本类型**：直接返回值
2. **结构体**：必须使用显式类型名或构造函数
3. **类**：必须使用 `new` 关键字
...
func getEmptyPoint() -> Point {
    // 3. 使用默认构造函数
    return Point();
}

// 错误示例
func getInvalidPoint() -> Point {
    // 错误：缺少类型名
    // return { x = 10, y = 20 };
...
| 场景             | 推荐写法                                     | 禁止写法                         | 备注            |
| :--------------- | :------------------------------------------- | :------------------------------- | :-------------- |
| 基本类型初始化   | `int x = 10;`                                | `int x { 10 };`                  | 保持简洁性      |
| 数组初始化       | `int[] a = [1, 2];`                          | `int[] a = {1, 2};`              | `[]` 专用于序列 |
| 结构体初始化     | `Point p = Point(1, 2);`                     | `Point p = { 1, 2 };`            | 必须显式类型名  |
| 结构体对象初始化 | `Point p = Point { x = 1, y = 2 };`          | `Point p = { x = 1, y = 2 };`    | 必须显式类型名  |
| 类初始化         | `Person^ p = new Person { Name = "Alice" };` | `Person p = { Name = "Alice" };` | 必须显式 `new`  |
| 返回语句         | `return Point(1, 2);`                        | `return { x = 1, y = 2 };`       | 必须显式类型名  |
| 返回空对象       | `return Point();`                            | `return {};`                     | 必须显式类型名  |

## 3. 总结表

| 类型          | 推荐写法                       | 禁止写法                                                 | 备注              |
| :------------ | :----------------------------- | :------------------------------------------------------- | :---------------- |
| **基本类型**  | `int x = 10;`                  | `int x(10);` <br> `int x { 10 };`                        | 保持 C 风格简单性 |
| **数组**      | `int[] a = [1, 2];`            | `int[] a = {1, 2};`                                      | `[]` 专用于序列   |
| **类 (Heap)** | `var p = new Person { Id=1 };` | `Person p = { Id=1 };`                                   | 必须显式 `new`    |
| **结构体**    | `Point p = Point(1, 2);`       | `Point p = { 1, 2 };` <br> `Point p = { x = 1, y = 2 };` | 必须显式类型名    |
| **返回语句**  | `return Point(1, 2);`          | `return { x = 1, y = 2 };` <br> `return {};`             | 必须显式类型名    |
