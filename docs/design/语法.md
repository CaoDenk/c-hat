...
// 字符串 (标准库类型)
string name = "C-Hat"; // 字面量是 literalview，隐式转换为 string
...
public class Player {
    ...
    // 不可变方法：函数后缀 ! (推荐)
    public func get_health()! -> int => _health;

    // 运算符重载：必须使用 self
    public func operator==(self!, Player other) -> bool {
...
*   **`self` 与不可变方法设计原则**：
    *   **普通成员函数**：`self` 可选，不关心可变性时可不写，与 Python/Rust 风格一致。
    *   **标记不可变性**：函数后缀 `func foo()!`（推荐）或 `self!` 参数，两种方式等价。
    *   **运算符重载**：**强制使用** `self`（或 `self!`），用于清晰表达左/右操作数。
    *   **取值方式**：隐式 `self` 和显式 `self` 都可直接访问成员，无语法差异。
